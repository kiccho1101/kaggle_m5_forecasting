...
with timer("basic price aggregations"):
    ...
    prices_df["fe_price_norm"] = prices_df["sell_price"] / prices_df["fe_price_max"]
    prices_df["fe_price_nunique"] = prices_df.groupby(["store_id", "item_id"])[
        "sell_price"
    ].transform("nunique")
    prices_df["fe_price_item_nunique"] = prices_df.groupby(["store_id", "sell_price"])[
        "item_id"
    ].transform("nunique")
...
with timer("calc price momentum"):
    prices_df["fe_price_momentum"] = prices_df["sell_price"] / prices_df.groupby(
        ["store_id", "item_id"]
    )["sell_price"].transform(lambda x: x.shift(1))
    prices_df["fe_price_momentum_m"] = prices_df["sell_price"] / prices_df.groupby(
        ["store_id", "item_id", "month"]
    )["sell_price"].transform("mean")
    prices_df["fe_price_momentum_y"] = prices_df["sell_price"] / prices_df.groupby(
...
# %%
prices_df.apply(lambda col: col.isna().sum(), axis=1)
