...
class WRMSSEEvaluator(object):
    ...
    def rmsse(self, valid_pred: pd.DataFrame, lv: int) -> pd.Series:
        valid_y = getattr(self, f"lv{lv}_valid_df")
        score = ((valid_y - valid_pred) ** 2).mean(axis=1)
        scale = getattr(self, f"lv{lv}_scale")
        return (score / scale).map(np.sqrt)

    def score(self, valid_pred: Union[pd.DataFrame, np.ndarray]) -> float:
        assert self.valid_df[self.valid_target_columns].shape == valid_pred.shape

        if isinstance(valid_pred, np.ndarray):
            valid_pred = pd.DataFrame(valid_pred, columns=self.valid_target_columns)

        valid_pred = pd.concat(
            [self.valid_df[self.id_columns], valid_pred], axis=1, sort=False
        )

        all_scores = []
        for i, group_id in enumerate(self.group_ids):
            lv_scores = self.rmsse(
                valid_pred.groupby(group_id)[self.valid_target_columns].sum(), i + 1
            )
